...
public class VelocityManager {
    ...
    //In order to get the Portlet actionURL in VM result, add this attribute in
    ...
    protected VelocityContext[] prepareChainedContexts(HttpServletRequest servletRequest, HttpServletResponse servletResponse, Map extraContext) {
        ...
        for (int i = 0; i < chainedContextNames.length; i++) {
            ...
            try {
                VelocityContext velocityContext = (VelocityContext) ObjectFactory.getObjectFactory().buildBean(className, extraContext);
     ...
     * initialization process, say by ServletDispatcher. this may be called
    ...
    public Properties loadConfiguration(ServletContext context) {
        ...
        //fixed for velocity.properties userdirectvie property --Added by Henry Hu
    ...
    /**
     * Initializes the ServletToolboxManager for this servlet's toolbox (if
     * any).
     */
     ...
     * additional VelocityContexts to chain to the the WebWorkVelocityContext.
     * The intent is to allow these contexts to store helper objects that the ui
     * developer may want access to. Examples of reasonable VelocityContexts
     * would be an IoCVelocityContext, a SpringReferenceVelocityContext, and a
